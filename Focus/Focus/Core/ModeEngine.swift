...
final class ModeEngine {
    ...
    var onModeChanged: ((FocusMode) -> Void)?
    ...
    func switchMode(to mode: FocusMode) {
    ...
    private func beginSession(mode: FocusMode, intention: String?, durationMinutes: Int? = nil) {
    ...
    private func handleSessionComplete() {
        ...
        let wasDeepWork = session.mode == FocusMode.deepWork.rawValue
        let wasBreak = session.mode == FocusMode.personalTime.rawValue && session.plannedDurationSeconds <= TimerDurations.breakMinutes * 60
