...
/// Runs inside the FocusHelper daemon (as root).
final class HostsFileManager: Sendable {
    ...
    private let managedHeader = AppConstants.focusManagedHeader
    private let managedFooter = AppConstants.focusManagedFooter

    private let logger = Logger(subsystem: "com.focus.helper", category: "hosts")
    ...
    func updateBlockedDomains(_ domains: [String]) throws {
        ...
        // 2. Parse: separate user section from Focus managed section
        ...
        if rename(tempPath, hostsPath) != 0 {
            ...
            throw FocusHelperError.renameFailed(errMsg)
        }
    ...
    /// Remove all Focus artifacts for clean uninstall
    ...
    private func generateManagedSection(domains: [String]) -> String {
        ...
        lines.append("# Do not edit this section. Managed by Focus app.")
    ...
    private func validateHostsFile(at path: String) throws {
        ...
        for (index, line) in lines.enumerated() {
            ...
            // Basic IP validation
            if !isValidIP(ip) {
                throw FocusHelperError.validationFailed(line: index + 1, content: trimmed)
            }
        }
    }
...
enum FocusHelperError: Error, Sendable {
