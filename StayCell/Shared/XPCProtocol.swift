...
/// Protocol for the XPC service provided by StayCellHelper daemon.
/// The daemon runs as root and owns all /etc/hosts writes.
@objc protocol StayCellHelperProtocol {
    ...
    /// Restore /etc/hosts from backup, removing all StayCell-managed entries.
